...
def simultaneous_record(serial_port_name, sample_rate=[1600, 8000], sample_length=5, channel=[3, 1], sensor='AM', plot=False, format='numpy'): 
    '''
    This function records data from serial port and save it as a wav file.
    '''
    ser = serial.Serial(serial_port_name, 115200, timeout=1)     # Create Serial link
    ...
    while (i <= samples[0] or j <= samples[1]):
      ...
      if id == b'\x00':
        ...
        data[1].extend(c)
    print("Stop recording.")
    real_sample_rate = [i/(time.time()-start_time), j/(time.time()-start_time)]
    ...
    if format == 'numpy':
          np.save(sensor + '/A_' + str(start_time) + "_" + str(int(real_sample_rate[0])) + ".npy", data[0])
          np.save(sensor + '/M_' + str(start_time) + "_" + str(int(real_sample_rate[1])) + ".npy", data[1])
    else:
          write(sensor + '/A_' + str(start_time) + "_" + str(int(real_sample_rate[0])) + ".wav", sample_rate[0], data[0])
          write(sensor + '/M_' + str(start_time) + "_" + str(int(real_sample_rate[1])) + ".wav", sample_rate[1], data[1])
    ser.close() 
    if plot:
        fig, axs = plt.subplots(2, 1)
        axs[0].plot(data[0])
