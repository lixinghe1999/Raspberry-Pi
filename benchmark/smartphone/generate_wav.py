...
pulse_duration = 0.03  # Duration of each pulse in seconds
pulse_interval = 0.01  # Time interval between pulses in seconds
...
for _ in range(num_pulses):
    ...
    window = np.hanning(int(pulse_duration * sample_rate))
    pulse_signal = pulse_signal * window
    # Add the pulse signal to the main signal
    signal[int(pulse_time * sample_rate): int(pulse_time * sample_rate)+int(pulse_duration * sample_rate) ] += pulse_signal
...
wavfile.write('FMCW.wav', sample_rate, signal)